...
class LIR_List: public CompilationResourceObj {
 ...
 public:
  ...
  // machine dependent
  template<typename T>
  void cmp_branch(LIR_Condition condition, LIR_Opr left, LIR_Opr right, T tgt, CodeEmitInfo* info = NULL);
  template<typename T>
  void cmp_branch(LIR_Condition condition, LIR_Opr left, int right, T tgt, CodeEmitInfo* info = NULL) {
    cmp_branch(condition, left, LIR_OprFact::intConst(right), tgt, info);
  }
  void cmp_branch(LIR_Condition condition, LIR_Opr left, LIR_Opr right, BlockBegin* block, BlockBegin* unordered);
